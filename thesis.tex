...
To better understand fuzzers, researchers traditionally classify them
along two orthogonal axes: the level of knowledge about the PUT that
they possess, and the strategy they use to generate test inputs.

\subsubsection*{Knowledge of the PUT}\label{knowledge-of-the-put}
\addcontentsline{toc}{subsubsection}{Knowledge of the PUT}

Fuzzers differ in how much information they leverage about the program
under test:
...
Black-box fuzzers treat the PUT as a black box, generating inputs
without insights into program internals. This makes them simple but
often less efficient in uncovering deep bugs.
...
Grey-box fuzzers strike a balance by collecting partial information,
such as execution coverage via lightweight instrumentation, enabling
more targeted input mutations that improve effectiveness.
...
White-box fuzzers exploit full program knowledge, using advanced
techniques like symbolic execution to methodically explore program
paths, but often at the cost of reduced scalability.

\subsubsection*{Test-case Generation
Strategy}\label{test-case-generation-strategy}
\addcontentsline{toc}{subsubsection}{Test-case Generation Strategy}
...
Produces inputs from a structural model or protocol description,
ensuring that test-cases are syntactically valid yet semantically
diverse.

\end{definition}

Generational fuzzing leverages knowledge of input formats (e.g., a BNF
grammar \autocite{backus1959}) to produce well-formed test cases derived
from formal specifications or models, improving the likelihood of
...
Mutational fuzzing begins with seeds and applies random or guided
mutations to explore nearby input space regions. Techniques like
coverage-guided fuzzing have greatly enhanced the efficiency of this
approach.

These two dimensions are often combined to tailor fuzzers to specific
scenarios. For example, AFL \autocite{afl} is a grey-box, mutational
fuzzer that uses coverage feedback to guide input mutations, while
Honggfuzz \autocite{honggfuzz} can operate as a grey-box generational
fuzzer when provided with grammar-based input models. This flexibility
allows fuzzers to adapt to varied testing goals and program
characteristics.
