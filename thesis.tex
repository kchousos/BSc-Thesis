...
Discovering vulnerabilities in the development stage instead of in
production.

Discovering vulnerabilities ourselves before attackers do.

Borrowing definitions from \autocite{manes2019}:
...
sampled from an input space (the ``fuzz input space'') that protrudes
...
Fuzz testing is the use of fuzzing to test if a PUT violates a security
policy.

\end{definition}

\begin{definition}[Fuzzer]\protect\hypertarget{def-fuzzer}{}\label{def-fuzzer}
...
A bug oracle is a program, perhaps as part of a fuzzer, that determines
...
A black-box fuzzer is a testing tool that inputs random or specified
data into a software application without knowledge of its internal
workings, aiming to uncover vulnerabilities or bugs by observing the
program's behavior in response to various inputs.
...
A white-box fuzzer is a testing tool that analyzes the internal
structure and logic of a program to generate test inputs. It uses
knowledge of the code, such as control flow and data paths, to
systematically explore all possible execution paths and identify
...
A grey-box fuzzer is a testing tool that combines aspects of both
black-box and white-box fuzzing. It has limited knowledge of the
internal workings of the application, often using some code coverage
information or program analysis to generate more targeted inputs,
...
Generationbased fuzzers produce test cases based on a given model that
describes the inputs expected by the PUT, e.g.~a Backus--Naur form (BNF)
grammar \autocite{backus1959}.
...
mutation-based fuzzers produce test cases by mutating a given seed
input.

\end{definition}

terminology: fuzz campaign (Definition~\ref{def-campaign}), harness,
driver, target, corpus
