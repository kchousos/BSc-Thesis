...
\chapter{Dateparse example}\label{dateparse-example}

The outputs from each phase of the OverHAuL process pertaining to the
dateparse project: the static analysis report
(Listing~\ref{lst-static}), sample function segments derived from
dateparse (Listing~\ref{lst-chunks}), the generated compilation command
(Listing~\ref{lst-compilation}), the bug-finding harness
(Listing~\ref{lst-harness}), the identified crash input
(Listing~\ref{lst-input}), and a portion of the harness output
(Listing~\ref{lst-output}).
