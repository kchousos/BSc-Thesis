...
existing work in automated fuzzing (\#sec-related). We demonstrate that
most current fuzzing systems either rely on pre-existing harnesses or
employ client code, often leaving validation and integration tasks to
the user. We then introduce the OverHAuL system, detailing its
architecture, the novel techniques used in its implementation, and their
contributions to advancing automated harness generation
(Chapter~\ref{sec-overhaul}). Finally, we assemble a benchmark dataset
of ten open-source C projects and rigorously evaluate OverHAuL's
performance (Chapter~\ref{sec-eval}-\ref{sec-results}).
