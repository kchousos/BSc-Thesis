...
\makeatother
\subtitle{Harnessing Automation for C Libraries with Large Language
Models}
...
{\centering
	...
	\begin{minipage}[t]{\textwidth}
		\begin{center}
			...
			{\LARGE \bfseries
				OverHAuL: Harnessing Automation for C Libraries with Large Language
Models
				\linebreak
			...
			{\bfseries
				%
				{\Large Konstantinos Chousos} \orcidlink{0009-0008-6063-7915}
				%
			}
...
\href{https://pergamos.lib.uoa.gr/uoa/dl/object/5300250}{Pergamos}.
...
\item
  ...
  generates effective fuzzing harnesses with a success rate of
  \textbf{81.25\%}.
\item
...
On the defensive tooling side, the security tool named \emph{Mayhem}
\autocite{avgerinos2018,cha2012}---developed by the company of the same
name, formerly known as ForAllSecure---has since been adopted by the US
Air Force, the Pentagon, Cloudflare, and numerous open-source
communities. It has found and facilitated the remediation of thousands
of previously unknown vulnerabilities, from errors in Cloudflare's
infrastructure to bugs in open-source projects like OpenWRT
...
\NormalTok{usage: overhaul [{-}h] [{-}c COMMIT] [{-}m MODEL] [{-}f FILES [FILES ...]] }
\NormalTok{[{-}o OUTPUT\_DIR] repo}

\NormalTok{Generate fuzzing harnesses for C/C++ projects}

\NormalTok{positional arguments:}
\NormalTok{  repo                  Link of a project\textquotesingle{}s git repo, for which to generate }
\NormalTok{                        a harness.}
...
root directory or exported in the shell environment:

\begin{Shaded}
\begin{Highlighting}[numbers=left,,]
\NormalTok{$ echo "OPENAI\_API\_KEY=\textless{}API{-}key{-}here\textgreater{}" \textgreater{}\textgreater{} .env}
\NormalTok{\# OR}
...
cloned. For a concrete example, we will use OverHAuL to create a new
fuzzing harness for \href{https://github.com/dvhar/dateparse}{dvhar's
dateparsing C library} and specify the LLM model to OpenAI's gpt-4.1
model. The resulting command and its output is presented in
...
\caption[OverHAuL execution on dateparse]{\label{fig-success}A
successful execution of OverHAuL, harnessing the ``dateparse'' library
using OpenAI's gpt-4.1 model. Debug statements are printed to showcase
the queries of the LLM agents to the codebase oracle
...
directory of OverHAuL. Following a successful execution, the project's
directory will contain a new folder named \texttt{harnesses}, which will
house all the generated harnesses formatted as
\texttt{harness\_n.c}---where \(n\) ranges from 1 to \(N-1\), with \(N\)
representing the total number of harnesses produced. The most recent and
verifiably correct harness will be designated simply as
\texttt{harness.c}. Additionally, the dateparse folder will include an
executable script named \texttt{overhaul.sh}, which contains the
compilation command necessary for the harness. A log file titled
\texttt{harness.out} will also be present, documenting the output from
the latest harness execution. Lastly and most importantly, there will be
at least one non-empty crash file included, serving as a witness to the
harness's correctness. In the following sections, the intermediary steps
between invocation and completion are disected and analyzed. The
dateparse project is used as a running example.
...
\caption[Static analysis report]{\label{lst-static}Static analysis
report (Flawfinder output) of dateparse.}

\centering{

\begin{Shaded}
\begin{Highlighting}[numbers=left,,]
\NormalTok{Flawfinder version 2.0.19, (C) 2001{-}2019 David A. Wheeler.}
\NormalTok{Number of rules (primarily dangerous function names) in C/C++ ruleset: 222}
\NormalTok{Examining ./dateparse.c}
\NormalTok{Examining ./dateparse.h}
\NormalTok{Examining ./test.c}

\NormalTok{FINAL RESULTS:}

\NormalTok{./dateparse.c:405:  [4] (buffer) strcpy:}
\NormalTok{  Does not check for buffer overflows when copying to destination [MS{-}banned]}
\NormalTok{  (CWE{-}120). Consider using snprintf, strcpy\_s, or strlcpy (warning: strncpy}
\NormalTok{  easily misused).}

\NormalTok{......}

\NormalTok{./dateparse.c:2192:  [1] (buffer) strlen:}
\NormalTok{  Does not handle strings that are not \textbackslash{}0{-}terminated; if given one it may}
\NormalTok{  perform an over{-}read (it could cause a crash if unprotected) (CWE{-}126).}

\NormalTok{ANALYSIS SUMMARY:}

\NormalTok{Hits = 64}
\NormalTok{Lines analyzed = 2719 in approximately 0.04 seconds (61234 lines/second)}
\NormalTok{Physical Source Lines of Code (SLOC) = 1966}
\NormalTok{Hits@level = [0]  15 [1]  28 [2]  31 [3]   0 [4]   5 [5]   0}
\NormalTok{Hits@level+ = [0+]  79 [1+]  64 [2+]  36 [3+]   5 [4+]   5 [5+]   0}
\NormalTok{Hits/KSLOC@level+ = [0+] 40.1831 [1+] 32.5534 [2+] 18.3113 [3+] 2.54323}
\NormalTok{    [4+] 2.54323 [5+]   0}
\NormalTok{Dot directories skipped = 1 ({-}{-}followdotdir overrides)}
\NormalTok{Minimum risk level = 1}

\NormalTok{Not every hit is necessarily a security vulnerability.}
\NormalTok{You can inhibit a report by adding a comment in this form:}
\NormalTok{// flawfinder: ignore}
\NormalTok{Make *sure* it\textquotesingle{}s a false positive!}
\NormalTok{You can use the option {-}{-}neverignore to show these.}

\NormalTok{There may be other security vulnerabilities; review your code!}
\NormalTok{See \textquotesingle{}Secure Programming HOWTO\textquotesingle{}}
\NormalTok{(https://dwheeler.com/secure{-}programs) for more information.}
\end{Highlighting}
\end{Shaded}
...
The codebase oracle is created in the following manner: The source code
is first chunked in function-level pieces by traversing the code's
Abstract Syntax Tree (AST) through Clang. Each chunk is represented by
an object with the function's signature, the corresponding filepath and
the function's body (see Listing~\ref{lst-chunks}). Afterwards, each
function body is turned into a vector embedding through an embedding
model. Each embedding is stored in the vector store. This structure is
created and used for easier and more semantically meaningful code
retrieval, and to also combat context window limitations present in
LLMs.

\begin{codelisting}

\caption[Codebase oracle samples]{\label{lst-chunks}Sample chunks
contained in dateparse's codebase oracle.}

\centering{

\begin{Shaded}
\begin{Highlighting}[numbers=left,,]
\NormalTok{File}\OperatorTok{:}\NormalTok{ dateparse}\OperatorTok{/}\NormalTok{dateparse}\OperatorTok{.}\NormalTok{c}
\NormalTok{Signature}\OperatorTok{:} \DataTypeTok{void} \OperatorTok{(}\KeywordTok{struct}\NormalTok{ parser }\OperatorTok{*,} \DataTypeTok{int}\OperatorTok{,} \DataTypeTok{int}\OperatorTok{)}
\NormalTok{Code}\OperatorTok{:}
\DataTypeTok{static} \DataTypeTok{void}\NormalTok{ setOffset}\OperatorTok{(}\KeywordTok{struct}\NormalTok{ parser}\OperatorTok{*}\NormalTok{ p}\OperatorTok{,} \DataTypeTok{int}\NormalTok{ i}\OperatorTok{,} \DataTypeTok{int}\NormalTok{ len}\OperatorTok{)\{}
\NormalTok{        strncpy}\OperatorTok{(}\NormalTok{p}\OperatorTok{{-}\textgreater{}}\NormalTok{offsetbuf}\OperatorTok{,}\NormalTok{ p}\OperatorTok{{-}\textgreater{}}\NormalTok{datestr}\OperatorTok{+}\NormalTok{i}\OperatorTok{,}\NormalTok{ len}\OperatorTok{);}
\NormalTok{        p}\OperatorTok{{-}\textgreater{}}\NormalTok{offsetbuf}\OperatorTok{[}\NormalTok{len}\OperatorTok{]} \OperatorTok{=} \DecValTok{0}\OperatorTok{;}
\OperatorTok{\}}


\NormalTok{File}\OperatorTok{:}\NormalTok{ dateparse}\OperatorTok{/}\NormalTok{dateparse}\OperatorTok{.}\NormalTok{c}
\NormalTok{Signature}\OperatorTok{:} \DataTypeTok{void} \OperatorTok{(}\KeywordTok{struct}\NormalTok{ parser }\OperatorTok{*,} \DataTypeTok{char} \OperatorTok{*)}
\NormalTok{Code}\OperatorTok{:}
\DataTypeTok{static} \DataTypeTok{void}\NormalTok{ setFullMonth}\OperatorTok{(}\KeywordTok{struct}\NormalTok{ parser}\OperatorTok{*}\NormalTok{ p}\OperatorTok{,} \DataTypeTok{char}\OperatorTok{*}\NormalTok{ month}\OperatorTok{)\{}
\NormalTok{        strcpy}\OperatorTok{(}\NormalTok{p}\OperatorTok{{-}\textgreater{}}\NormalTok{mobuf}\OperatorTok{,}\NormalTok{ month}\OperatorTok{);}
\OperatorTok{\}}


\NormalTok{File}\OperatorTok{:}\NormalTok{ dateparse}\OperatorTok{/}\NormalTok{dateparse}\OperatorTok{.}\NormalTok{c}
\NormalTok{Signature}\OperatorTok{:} \DataTypeTok{int} \OperatorTok{(}\DataTypeTok{const} \DataTypeTok{char} \OperatorTok{*,} \DataTypeTok{long} \DataTypeTok{long} \OperatorTok{*,} \DataTypeTok{int} \OperatorTok{*,} \DataTypeTok{int}\OperatorTok{)}
\NormalTok{Code}\OperatorTok{:}
...
project's directory. An evaluation-passing harness generated for the
dateparse project is presented in Listing~\ref{lst-harness}, along with
the associated crash input and execution output displayed in
Listing~\ref{lst-input} and Listing~\ref{lst-output}, respectively.

\begin{codelisting}

\caption[Sample dateparse harness]{\label{lst-harness}A crash-finding
harness for dateparse, generated through OverHAuL (some comments were
removed).}
...
\CommentTok{// ...}
\KeywordTok{struct}\NormalTok{ parser }\OperatorTok{\{}
    \DataTypeTok{char}\NormalTok{ mobuf}\OperatorTok{[}\DecValTok{16}\OperatorTok{];}
\OperatorTok{\};}
...
\DataTypeTok{int}\NormalTok{ LLVMFuzzerTestOneInput}\OperatorTok{(}\DataTypeTok{const} \DataTypeTok{uint8\_t} \OperatorTok{*}\NormalTok{data}\OperatorTok{,} \DataTypeTok{size\_t}\NormalTok{ size}\OperatorTok{)} \OperatorTok{\{}
    \CommentTok{// Allocate a parser instance on the heap.}
    \KeywordTok{struct}\NormalTok{ parser }\OperatorTok{*}\NormalTok{p }\OperatorTok{=} \OperatorTok{(}\KeywordTok{struct}\NormalTok{ parser}\OperatorTok{*)}\NormalTok{malloc}\OperatorTok{(}\KeywordTok{sizeof}\OperatorTok{(}\KeywordTok{struct}\NormalTok{ parser}\OperatorTok{));}
    \ControlFlowTok{if} \OperatorTok{(!}\NormalTok{p}\OperatorTok{)} \OperatorTok{\{}
        \ControlFlowTok{return} \DecValTok{0}\OperatorTok{;}
    \OperatorTok{\}}
    \CommentTok{// Initialize parser with zeros.}
\NormalTok{    memset}\OperatorTok{(}\NormalTok{p}\OperatorTok{,} \DecValTok{0}\OperatorTok{,} \KeywordTok{sizeof}\OperatorTok{(}\KeywordTok{struct}\NormalTok{ parser}\OperatorTok{));}
    \CommentTok{// Prepare month string input: ensure null{-}terminated string for strcpy.}
    \DataTypeTok{char} \OperatorTok{*}\NormalTok{month }\OperatorTok{=} \OperatorTok{(}\DataTypeTok{char}\OperatorTok{*)}\NormalTok{malloc}\OperatorTok{(}\NormalTok{size }\OperatorTok{+} \DecValTok{1}\OperatorTok{);}
    \ControlFlowTok{if} \OperatorTok{(!}\NormalTok{month}\OperatorTok{)} \OperatorTok{\{}
\NormalTok{        free}\OperatorTok{(}\NormalTok{p}\OperatorTok{);}
        \ControlFlowTok{return} \DecValTok{0}\OperatorTok{;}
    \OperatorTok{\}}
\NormalTok{    memcpy}\OperatorTok{(}\NormalTok{month}\OperatorTok{,}\NormalTok{ data}\OperatorTok{,}\NormalTok{ size}\OperatorTok{);}
\NormalTok{    month}\OperatorTok{[}\NormalTok{size}\OperatorTok{]} \OperatorTok{=} \CharTok{\textquotesingle{}}\SpecialCharTok{\textbackslash{}0}\CharTok{\textquotesingle{}}\OperatorTok{;}  \CommentTok{// Null terminate to avoid overread in strcpy.}
    \CommentTok{// Call the vulnerable function with fuzzed month string.}
\NormalTok{    setFullMonth}\OperatorTok{(}\NormalTok{p}\OperatorTok{,}\NormalTok{ month}\OperatorTok{);}
    \CommentTok{// Cleanup}
\NormalTok{    free}\OperatorTok{(}\NormalTok{month}\OperatorTok{);}
\NormalTok{    free}\OperatorTok{(}\NormalTok{p}\OperatorTok{);}
    \ControlFlowTok{return} \DecValTok{0}\OperatorTok{;}
\OperatorTok{\}}
\end{Highlighting}
...
\caption[Sample dateparse crash input]{\label{lst-input}An input string
that crashes the harness in Listing~\ref{lst-harness}. What is shown is
its \texttt{xxd} output.}
...
\caption[Sample dateparse harness output]{\label{lst-output}The output
of the harness in Listing~\ref{lst-harness} when executed with
Listing~\ref{lst-input} as input.}

\centering{

\begin{Shaded}
\begin{Highlighting}[numbers=left,,]
\NormalTok{INFO: Running with entropic power schedule (0xFF, 100).}
\NormalTok{INFO: Seed: 2365219758}
\NormalTok{INFO: Loaded 1 modules   (3723 inline 8{-}bit counters): 3723 [0x67ccc0,}
\NormalTok{      0x67db4b),}
\NormalTok{INFO: Loaded 1 PC tables (3723 PCs): 3723 [0x618d40,0x6275f0),}
\NormalTok{./harness: Running 1 inputs 1 time(s) each.}
\NormalTok{Running: crash{-}7fd6f4dd5d39420d6f7887ff995b4e855ae90c16}
\NormalTok{=================================================================}
\NormalTok{==10973==ERROR: AddressSanitizer: heap{-}buffer{-}overflow on address}
\NormalTok{0x7bcece9e00a0 at pc 0x000000526c0e bp 0x7fff3dc0aa20 sp 0x7fff3dc0a1d8}
\NormalTok{WRITE of size 18 at 0x7bcece9e00a0 thread T0}
\NormalTok{    \#0 0x000000526c0d in strcpy}
\NormalTok{       (/home/kchou/Bin/Repos/kchousos/OverHAuL/output/dateparse/harness}
\NormalTok{       +0x526c0d) (BuildId: d658684b8726dc7e8e768089710d13c96cfc81f0)}
\NormalTok{    \#1 0x000000585555 in setFullMonth}
\NormalTok{       /home/kchou/Bin/Repos/kchousos/OverHAuL/output/dateparse/harnesses}
\NormalTok{       /harness.c:18:2}
\NormalTok{    \#2 0x0000005853fd in LLVMFuzzerTestOneInput}
\NormalTok{       /home/kchou/Bin/Repos/kchousos/OverHAuL/output/dateparse/harnesses}
\NormalTok{       /harness.c:41:2}
\NormalTok{...}
\NormalTok{SUMMARY: AddressSanitizer: heap{-}buffer{-}overflow}
\NormalTok{         (/home/kchou/Bin/Repos/kchousos/OverHAuL/output/dateparse/harness}
\NormalTok{         +0x526c0d) (BuildId: d658684b8726dc7e8e768089710d13c96cfc81f0) in}
\NormalTok{         strcpy}
\NormalTok{Shadow bytes around the buggy address:}
\NormalTok{  0x7bcece9dfe00: 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00}
\NormalTok{  0x7bcece9dfe80: 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00}
\NormalTok{  0x7bcece9dff00: 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00}
\NormalTok{  0x7bcece9dff80: 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00}
\NormalTok{  0x7bcece9e0000: fa fa 00 00 fa fa 00 fa fa fa 00 fa fa fa 00 fa}
\NormalTok{=\textgreater{}0x7bcece9e0080: fa fa 00 00[fa]fa fa fa fa fa fa fa fa fa fa fa}
\NormalTok{  0x7bcece9e0100: fa fa fa fa fa fa fa fa fa fa fa fa fa fa fa fa}
\NormalTok{  0x7bcece9e0180: fa fa fa fa fa fa fa fa fa fa fa fa fa fa fa fa}
\NormalTok{  0x7bcece9e0200: fa fa fa fa fa fa fa fa fa fa fa fa fa fa fa fa}
\NormalTok{  0x7bcece9e0280: fa fa fa fa fa fa fa fa fa fa fa fa fa fa fa fa}
\NormalTok{  0x7bcece9e0300: fa fa fa fa fa fa fa fa fa fa fa fa fa fa fa fa}
\NormalTok{Shadow byte legend (one shadow byte represents 8 application bytes):}
\end{Highlighting}
\end{Shaded}
...
\NormalTok{math(question}\OperatorTok{=}\NormalTok{(}
    \StringTok{"Two dice are tossed. What is the probability that the sum equals two?"}
\NormalTok{))}
\end{Highlighting}
\end{Shaded}
...
corresponding GitHub repository. Each is followed by a short
description, its GitHub stars count and its Significant Lines of Code
(SLOC), as of July 18th, 2025.}\label{tbl-projects}\tabularnewline
\toprule\noalign{}
...
The outcomes of the benchmark experiments are shown in
Figure~\ref{fig-results}. To ensure the reliability of these results,
each reported crash was manually validated to confirm that it stemmed
from genuine defects within the target library, rather than issues of
the generated harness. An iteration heatmap was also generated for the
verifiably fuzzed projects, displayed in Figure~\ref{fig-iterations}.
With these validated findings, we are now positioned to address the
initial research questions posed in this chapter.
...
input. On the other hand, a yellow/0 block indicates that while a
compilable harness was produced, no crash input was found within the
five-minute execution period. Finally, an orange/-2 block means that the
crash that was found derives from errors in the harness itself.
AImportantly, there are no red/-1 blocks, which would indicate cases
where a compilable harness could not be generated.}
...
\pandocbounded{\includegraphics[keepaspectratio]{chapters/../resources/iterations.png}}

}

\caption[Iterations Heatmap]{\label{fig-iterations}This heatmap
illustrates the number of iterations required for each project to be
successfully harnessed, as determined by the benchmark results. Higher
color intensity corresponds to a greater number of iterations needed for
successful harnessing. Cells left blank signify instances where no valid
harness was generated.}
...
OverHAuL demonstrates a strong capability in generating working
harnesses for previously unfuzzed C projects. In benchmark evaluations,
it achieved a success rate of \textbf{81.25\%} in producing fuzzing
harnesses that were effective at uncovering crash-inducing inputs in
target programs. Notably, all harnesses generated by OverHAuL were valid
C programs---an improvement over prior methods such as OSS-Fuzz-Gen
\autocite{oss-fuzz-gen}, which occasionally outputs the LLM's markdown
answers instead. The harnesses consistently utilized existing functions
obtained from the codebase oracle and interacted appropriately with the
Library Under Test's API, with only minimal instances of irrelevant or
hallucinated code observed. While the potential exists for
non-compilable harnesses to be generated, the benchmark results included
no such cases, underscoring the significance and effectiveness of
compilation feedback and the integrated ``fixer'' agent in OverHAuL's
workflow. These findings collectively indicate that OverHAuL is
effective at generating robust, valid, and meaningful harnesses for C
projects lacking previous fuzzing infrastructure.
...
In examining the characteristics of the generated harnesses, we observe
several notable patterns. The harnesses are typically well-commented, a
result of explicit instructions given to the language models. They are
designed to target various levels of the library's functionality. In
some cases, they focus on higher-level entry point functions
(Section~\ref{sec-dateparse}), while in other instances, they
concentrate on more narrowly scoped internal functions
(Listing~\ref{lst-harness}). Usually the generated fuzz targets are
clear and closely resemble the kind of harnesses a skilled software
engineer might write. These harnesses make appropriate and sensible use
of the target API, as illustrated in examples such as
Listing~\ref{lst-harness}. However, some harnesses do exhibit the use of
unexplained constants or idiosyncratic control flow constructs, which
can hinder comprehensibility and may introduce errors. Additionally, we
find that the characteristics of generated harnesses can vary
substantially across different projects and even between runs, with
differences evident in both their size and complexity (see
Appendix~\ref{sec-sample-harnesses}). Overall, while the generated
harnesses often echo the structure and intent of man-made harnesses,
inconsistencies and occasional inexplicable design choices distinguish
them from their manually written counterparts.
...
The significance of the iterative feedback loop is clearly demonstrated
by the results presented in Figure~\ref{fig-iterations}. Analysis of the
heatmap reveals that earlier versions of OverHAuL, which employed a
one-shot approach to harness generation, achieved a success rate of only
28.75\%. In contrast, the current implementation shows that 42 out of 65
projects successfully fuzzed (64.62\%) did not produce a successful
harness in the initial attempt and therefore benefited from the
iterative feedback process. Notably, two projects (3.07\%) required the
full allocation of eight iterations, underscoring the necessity of
maintaining a generous iteration budget to maximize effectiveness.
...
\section{dvhar/dateparse}\label{sec-dateparse}
