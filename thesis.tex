...
\textbf{Fuzzing} is an automated software-testing technique in which a
...
More rigorously---borrowing Manes et al.'s definitions
\autocite{manes2019}:
...
An individual execution (or bounded set of executions) that meets this
definition is a \emph{fuzzing run}. When such runs are performed
systematically and at scale to test whether the PUT violates a specified
security policy, we speak of \textbf{fuzz testing} (or simply
\emph{fuzzing} in common parlance):
...
A \textbf{fuzzer engine} orchestrates a \textbf{fuzz campaign}---one
concrete instantiation of a fuzzer running against a single PUT under a
particular policy:
...
During each execution the \textbf{bug oracle} decides whether the
observed behaviour constitutes a policy violation:
...
In practice, many oracles are based on runtime instrumentation such as
fatal POSIX signals (e.g., \texttt{SIGSEGV}) or sanitizers like
AddressSanitizer (ASan) \autocite{serebryany2012}, as used by LibFuzzer
\autocite{libfuzzer}.

\subsection{Seeds, Mutation, and the Seed-Selection
Problem}\label{seeds-mutation-and-the-seed-selection-problem}

A campaign usually begins with one or more \textbf{seeds}---well-formed
inputs that belong to the PUT's expected input space. The fuzzer mutates
these seeds to explore the larger fuzz input space:
...
Selecting an initial corpus that leads to fast, wide code-coverage is
non-trivial and has been studied as the \emph{seed-selection problem}
\autocite{rebert2014}.

\subsection{Taxonomies of Fuzzing}\label{taxonomies-of-fuzzing}

Fuzzers are traditionally classified along two orthogonal dimensions.
Namely, in regards to the knowledge of the PUT they have access to and
the strategy used for the input generation.

\subsubsection{Knowledge of the PUT}\label{knowledge-of-the-put}

\begin{definition}[Black-box
fuzzer]\protect\hypertarget{def-blackbox}{}\label{def-blackbox}

Operates solely on program binaries, with no knowledge of internal
structure; input generation is guided only by external observations.

\end{definition}

\begin{definition}[Grey-box
fuzzer]\protect\hypertarget{def-greybox}{}\label{def-greybox}

Gains limited insight---typically lightweight coverage metrics---via
instrumentation, allowing more informed mutations while retaining
scalability.

\end{definition}

\begin{definition}[White-box
fuzzer]\protect\hypertarget{def-whitebox}{}\label{def-whitebox}

Has full source-level visibility and employs heavy program analysis
(symbolic execution, constraint solving, etc.) to systematically
enumerate paths.

\end{definition}

\subsubsection{Test-case Generation
Strategy}\label{test-case-generation-strategy}

\begin{definition}[Generational
fuzzing]\protect\hypertarget{def-generational}{}\label{def-generational}

Produces inputs from a structural model (e.g., a BNF grammar
\autocite{backus1959}) or protocol description, ensuring that test-cases
are syntactically valid yet semantically diverse.

\end{definition}

\begin{definition}[Mutational
fuzzing]\protect\hypertarget{def-mutational}{}\label{def-mutational}

Starts from existing seeds and applies stochastic mutations (bit-flips,
block insertions, splice operations). Coverage-guided mutational fuzzers
such as AFL have proved especially effective.

\end{definition}

These axes can be combined: e.g., AFL \autocite{afl} is a grey-box,
mutational fuzzer; Honggfuzz \autocite{honggfuzz} with a grammar
description becomes grey-box generational.

\subsection{Why Fuzz?}\label{why-fuzz}
...
arose from a buffer over-read in OpenSSL
\autocite{theopensslproject2025} introduced on 1 February 2012 and
unnoticed until 1 April 2014. Post-mortem analyses showed that a simple
...
variants were discovered by Google's Michał Zalewski using fuzzing in
late 2014 \autocite{saarinen2014}.
...
\subsection{How to Fuzz?}\label{how-to-fuzz}

\subsection{Fuzzer engines}\label{fuzzer-engines}

C/C++: AFL \autocite{afl} \& AFL++ \autocite[++]{afl}. LibFuzzer
\autocite{libfuzzer}. \autocite{honggfuzz}.

Python: Atheris \autocite{atheris}.

\begin{quote}
LibFuzzer is an in-process, coverage-guided, evolutionary fuzzing
engine. LibFuzzer is linked with the library under test, and feeds
fuzzed inputs to the library via a specific fuzzing entrypoint (fuzz
target). - In-process, coverage-guided, mutation-based fuzzer.
\end{quote}

Used to fuzz library functions. The programmer writes a fuzz target to
test their implementation.

\begin{definition}[Fuzz
target]\protect\hypertarget{def-target}{}\label{def-target}

A function that accepts an array of bytes and does something interesting
with these bytes using the API under test \autocite{libfuzzer}.
...
\NormalTok{  DoSomethingWithData}\OperatorTok{(}\NormalTok{Data}\OperatorTok{,}\NormalTok{ Size}\OperatorTok{);}
...
\tightlist
\item
  \emph{American Fuzzy Lop} (AFL) \autocite{afl}.
  ...
  \item
    ...
    support.
  \item
    Superseded by \emph{AFL++} \autocite{aflpp}.
  \end{itemize}
\end{itemize}

AFL fuzzes programs/binaries. The inputs are taken from the
\texttt{seeds\_dir} and their mutations.

\begin{Shaded}
\begin{Highlighting}[numbers=left,,]
\ExtensionTok{$}\NormalTok{ ./afl{-}fuzz }\AttributeTok{{-}i}\NormalTok{ seeds\_dir }\AttributeTok{{-}o}\NormalTok{ output\_dir }\AttributeTok{{-}{-}}\NormalTok{ /path/to/tested/program}
\end{Highlighting}
\end{Shaded}
...
Μπορεί επίσης να χρησιμοποιηθεί για fuzzing βιβλιοθηκών κτλ., απλά αντί
για \texttt{LLVMFuzzerTestOneInput} έχουμε την \texttt{main}.

Μπορεί να χρησιμοποιήσει και \texttt{LLVMFuzzerTestOneInput} harnesses.
...
\autocite{li2022} Transformers \autocite{vaswani2023}
...
\tightlist
\item
  Closed-source ChatGPT, Claude, Gemini \autocite{chatgpt,claude,gemini}
\item
  Open-source (i.e.~open-weights)
\end{itemize}

Llama\{1,..,3\}, Deepseek \autocite{grattafiori2024,deepseek-ai2025}
...
\item
  Zero-shot
\item
  Few-shot \autocite{brown2020}
\item
  RAG \autocite{lewis2021}
\item
  chain of thought \autocite{chainofthought}
\item
  tree of thought \autocite{yao2023}
\item
...
\subsection{For coding}\label{for-coding}

LLM-assisted IDEs \autocite{cursor,ghcopilot,chen2021} Vibecoding
\autocite{sarkar2025}

\subsection{For fuzzing}\label{for-fuzzing}

They don\textquotesingle t work
