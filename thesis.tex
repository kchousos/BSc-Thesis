...
diverging from seeming human-written. Additionally, the generated
harnesses differ significantly across projects and executions, both in
size and complexity (Appendix~\ref{sec-sample-harnesses}). RQ2's answer
remains an unclear ``it depends'', given the variance in OverHAuL's
results.
...
\CommentTok{// No artificial size limit to allow fuzzer to explore full}
\CommentTok{// input size for boundaries}
...
\NormalTok{    date\_t parsed\_date }\OperatorTok{=} \DecValTok{0}\OperatorTok{;}
    ...
    \CommentTok{// Array of string lengths targeting boundary }
    \CommentTok{// (including 0 = internal strlen)}
