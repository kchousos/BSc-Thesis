...
Finally, any crashes triggered during fuzzing are subjected to a reasoning process based on chain-of-thought prompting (@sec-prompting). To help determine their severity and root cause, CKGFuzzer consults an LLM-generated knowledge base containing real-world examples of vulnerabilities mapped to known CWE entries.

## PromptFuzz

PromptFuzz [@lyu2024] constitutes a framework for automatically generating fuzz drivers using LLMs, with a novel focus on *prompt mutation* to improve coverage. Its aim is to explore more of the API surface with each prompt iteration. It is implemented in Rust and targets C libraries.
